...
import HotelFormModal from "@/components/modal/hotelFormModal";
...
export default function Hotel() {
  const { hotels, meta } = usePage<{ 
    hotels: Hotel[],
    meta: MetaPagination
  }>().props;
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedPost, setSelectedPost] = useState<Hotel | null>(null);
  ...
  }, [searchTerm, hotels, isInitialLoading]);

  const openModal = (post: Hotel | null = null) => {
  ...
  const handleDelete = () => {
    if (isBulkDelete) {
      // Handle bulk delete
      setIsDeleting(true);
      router.post('/hotel/bulk-delete', { ids: selectedItems }, {
        onSuccess: () => {
      ...
      setIsDeleting(true);
      router.delete(`/hotel/${itemToDelete}`, {
        onSuccess: () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    router.get('/hotel', { search: searchTerm, page: 1 }, {
      preserveState: true,
      onSuccess: () => {
        setCurrentPage(1);
        setIsLoading(false);
  ...
  const handlePageChange = (page: number) => {
    setIsLoading(true);
    router.get('/hotel', { search: searchTerm, page }, {
      preserveState: true,
      onSuccess: () => {
        setCurrentPage(page);
        setIsLoading(false);
  ...
  const exportToExcel = () => {
    setIsLoading(true);
    window.location.href = `/hotel-export${searchTerm ? `?search=${searchTerm}` : ''}`;
    setTimeout(() => {
      setIsLoading(false);
  ...
  const exportToPdf = () => {
    setIsLoading(true);
    window.location.href = `/hotel-export-pdf${searchTerm ? `?search=${searchTerm}` : ''}`;
    setTimeout(() => {
      setIsLoading(false);
  ...
  return (
    <AppLayout>
                      ...
                      <TableCell>
                        <Badge
                          className={`text-sm font-medium px-2 py-0.5 rounded-full ${
                            post.city?.toLowerCase().includes("makkah")
                              ? "bg-yellow-100 text-yellow-800"
                              : post.city?.toLowerCase().includes("medina")
                          ...
                          {post.rating ? (
                            <div className="flex flex-col space-y-1 mt-1">
                              ...
                              <Rating
                                ...
                                itemStyles={{
                                  itemShapes: Star,
                                  activeFillColor: ratingLabels[Math.round(parseFloat(post.rating) || 0)]?.color || "#FFC107",
                              ...
                              <div className="flex items-center">
                                <div 
                                  ...
                                  style={{ backgroundColor: ratingLabels[Math.round(parseFloat(post.rating) || 0)]?.color || "#FFC107" }}
                                ></div>
                                <span className="text-xs">
                                  {post.rating} - {ratingLabels[Math.round(parseFloat(post.rating) || 0)]?.label || ""}
                                </span>
                              </div>
                            </div>
                          ) : (
      ...
      <HotelFormModal isOpen={isModalOpen} closeModal={() => setIsModalOpen(false)} post={selectedPost} />
