...
import { Pencil, Trash, Plus, Search, Download, ChevronLeft, ChevronRight } from "lucide-react";
import Swal from "sweetalert2";

export default function WebOption() {
  const { webOptions, meta } = usePage<{ 
    webOptions: { id: number; name: string; value: string; path_file?: string }[],
    meta: { 
      total: number, 
      per_page: number, 
      current_page: number, 
  ...
  const [filteredData, setFilteredData] = useState(webOptions);
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            {selectedItems.length > 0 && (
              <button 
                ...
                disabled={isLoading}
              >
                <Trash size={16} /> Hapus Terpilih ({selectedItems.length})
              </button>
            )}
          </div>
        </div>
